...
const BalancerFactory = require('./abis/balancerContracts.json');
...
const runBot = async () => {
  ...
  const loadPairs = async () => {
    balancerToken0Token1 = new ethers.Contract(
      await balancerFactory.getPair(token0Address, token1Address), /* how to deal with pairs here, what is their function? */
      BalancerFactory.abi, wallet,
    );
  ...
  provider.on('block', async (blockNumber) => {
    try {
      ...
      const balancerReserves = await balancerToken0Token1.getNumTokens(); /* https://github.com/balancer-labs/balancer-core/blob/master/contracts/BPool.sol#L115 */
